...
class GameScene: SKScene , SKPhysicsContactDelegate , AVAudioPlayerDelegate{
    ...
    var itemANode:SKNode!
    var itemB:SKSpriteNode!

    //BGM用のパラメーターを作成。
    var music:SKAudioNode!
    var audioPlayer: AVAudioPlayer!

    let audioPath = NSURL(fileURLWithPath: Bundle.main.path(forResource: "kira2", ofType: "mp3")!)
    let audioPathItem = NSURL(fileURLWithPath: Bundle.main.path(forResource: "decision26", ofType: "mp3")!)
    let audioPathGameover = NSURL(fileURLWithPath: Bundle.main.path(forResource: "hyun1", ofType: "mp3")!)
    ...
    let itemCategoryA: UInt32 = 1 << 4      // 0...10000
    ...
    func setupBird() {
        ...
        bird.physicsBody?.contactTestBitMask = groundCategory | wallCategory | itemCategoryA
    ...
    func setupItem() {
        ...
        // りんごの画像を2種類読み込む
        let ringo = SKTexture(imageNamed: "fruit_ringo.png")
        ringo.filteringMode = .linear
        let dokuringo = SKTexture(imageNamed: "apple_doku_ringo.png")
        dokuringo.filteringMode = .nearest
        
        // 移動する距離を計算
        let movingDistance = CGFloat(self.frame.size.width + ringo.size().width)
        ...
        let creatItemAnimation = SKAction.run({
            // 壁関連のノードを乗せるノードを作成
            let item = SKNode()
            item.position = CGPoint(x: self.frame.size.width + ringo.size().width / 2, y: 0)
            ...
            // アイテムアップ用のノード
            let itemNode = SKNode()
            itemNode.position = CGPoint(x: 0, y: slit_length * 5 )
            //scoreNode.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: upper.size.width, height: self.frame.size.height))
            itemNode.physicsBody?.isDynamic = false
            itemNode.physicsBody?.categoryBitMask = self.scoreCategory
        ...
        itemANode.run(repeatForeverAnimation)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
        ...
        } else if (contact.bodyA.categoryBitMask & itemCategoryA) == itemCategoryA || (contact.bodyB.categoryBitMask & itemCategoryA) == itemCategoryA {
            // アイテムと衝突した
            print("ItemGet")
            score += 3
            scoreLabelNode.text = "Score:\(score)"
            ...
            // 効果音を再生する
            do {
                audioPlayer = try AVAudioPlayer(contentsOf: audioPathItem as URL)
                audioPlayer.delegate = self
                audioPlayer.play()
            }
                // playerを作成した時にエラーがthrowされたらこっち来る
